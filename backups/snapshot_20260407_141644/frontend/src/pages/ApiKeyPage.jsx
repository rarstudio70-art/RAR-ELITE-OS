...
export default function ApiKeyPage() {
  ...
  const handleRequest = async (fn, label) => {
    try {
      if (!email) {
        notify("Enter an email first.", "error");
        return;
      }
  ...
  const createKey = () =>
    handleRequest(async () => {
      const res = await fetch(
        `${backend}/api/key/create?owner_email=${encodeURIComponent(email)}`,
        { method: "POST" }
      );
  ...
  const getInfo = () =>
    handleRequest(async () => {
      const res = await fetch(
        `${backend}/api/key/info?owner_email=${encodeURIComponent(email)}`
      );
      if (!res.ok) {
        notify("No active key found for this email.", "error");
        return;
      }
      const data = await res.json();
      setApiKey(data.api_key);
  ...
  const regenerate = () =>
    handleRequest(async () => {
      const res = await fetch(
        `${backend}/api/key/regenerate?owner_email=${encodeURIComponent(
          email
        )}`,
        { method: "POST" }
      );
  ...
  const revoke = () =>
    handleRequest(async () => {
      const res = await fetch(
        `${backend}/api/key/revoke?owner_email=${encodeURIComponent(email)}`,
        { method: "POST" }
      );
      if (!res.ok) {
          ...
          <div className="relative rounded-xl border border-cyan-500/40 bg-slate-950/70 backdrop-blur-md shadow-[0_0_40px_rgba(0,255,255,0.25)] overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-cyan-300/80">
                  OWNER EMAIL
                </label>
                <input
                  className="w-full rounded border border-cyan-500/40 bg-black/60 px-3 py-2 text-sm font-mono text-cyan-100 outline-none focus:border-cyan-300 focus:ring-1 focus:ring-cyan-400/70"
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded border border-cyan-500/40 bg-black/70 px-3 py-2 font-mono text-xs text-cyan-100 min-h-[40px] flex items-center">
                  {apiKey ? maskedKey : "No active key bound to this email."}
                </div>
              </div>
            </div>
          </div>
